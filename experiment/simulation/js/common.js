...
function calculate_temperature() {
    for (let i = 0; i < main_table_data.length; i++) {
        if (i == 0) {
            main_table_data[i][5] = const_temp * (Math.pow((main_table_data[i][4] / const_resistance), 0.741));
        }
        else {
            main_table_data[i][5] = main_table_data[i - 1][5] * (Math.pow((main_table_data[i][4] / const_resistance), 0.741));
        }
    }
